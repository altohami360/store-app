...
@section('page-title', __('admin.edit_faq'))
...
<div class="max-w-4xl">
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <form action="{{ route('admin.faqs.update', $faq) }}" method="POST" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="text"
                       name="question[en]"
                       id="question_en"
                       value="{{ old('question.en', $faq->getTranslation('question', 'en')) }}"
                       required
            ...
            <div>
                ...
                <input type="text"
                       name="question[ar]"
                       id="question_ar"
                       value="{{ old('question.ar', $faq->getTranslation('question', 'ar')) }}"
                       required
                       dir="rtl"
            ...
            <div>
                ...
                <textarea name="answer[en]"
                          ...
                          rows="5"
                          required
                          class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent @error('answer.en') border-red-500 @enderror">{{ old('answer.en', $faq->getTranslation('answer', 'en')) }}</textarea>
                @error('answer.en')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea name="answer[ar]"
                          ...
                          required
                          dir="rtl"
                          class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent @error('answer.ar') border-red-500 @enderror">{{ old('answer.ar', $faq->getTranslation('answer', 'ar')) }}</textarea>
                @error('answer.ar')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
